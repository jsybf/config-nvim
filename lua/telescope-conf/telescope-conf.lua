...
return {
  ...
  config = function()
    ...
    local bookmark_picker = require 'telescope-conf.folder-bookmark-picker'
    ...
    local function find_in_bookmark_dir()
      ...
      local bookmark_dir = {
        snippet = '~/gitp/dev/snippet',
      }

      builtin.live_grep {
        cwd = bookmark_dir[bookmark_name],
        prompt_title = 'Live Grep in bookmark',
      }
    end

    vim.keymap.set('n', '<leader><leader>', find_in_bookmark_dir)
